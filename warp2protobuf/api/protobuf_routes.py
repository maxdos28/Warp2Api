...
async def send_to_warp_api_parsed(
    ...
    try:
        ...
        if "input" in actual_data:
            logger.debug(f"[API] Original data - Has user_inputs: {'user_inputs' in actual_data.get('input', {})}")
            logger.debug(f"[API] Original data - Has context: {'context' in actual_data.get('input', {})}")
        ...
        # 调试：sanitize 后
        if "input" in actual_data:
            logger.debug(f"[API] After sanitize - Has user_inputs: {'user_inputs' in actual_data.get('input', {})}")
            logger.debug(f"[API] After sanitize - Has context: {'context' in actual_data.get('input', {})}")
            
        actual_data = _encode_smd_inplace(actual_data)
        
        # 调试：encode_smd 后
        if "input" in actual_data:
            logger.debug(f"[API] After encode_smd - Has user_inputs: {'user_inputs' in actual_data.get('input', {})}")
            logger.debug(f"[API] After encode_smd - Has context: {'context' in actual_data.get('input', {})}")
        ...
        # 调试：prepare_data 后
        if "input" in actual_data:
            logger.debug(f"[API] After prepare_data - Has user_inputs: {'user_inputs' in actual_data.get('input', {})}")
            logger.debug(f"[API] After prepare_data - Has context: {'context' in actual_data.get('input', {})}")
